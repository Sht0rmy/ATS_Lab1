...
public:
    ...
    VALUE* GetValue (const char* key) {
        ...
        if (salt < 0)
            ...
            dataIndex = Hash(key, strlen(key), (unsigned int)salt) % m_data.size();
...
private:
    ...
    unsigned int FirstHash (const char* key) {
        return Hash(key, strlen(key), 435) & m_bucketMask;
    }
        ...
        for (int salt = 0; ; ++salt) {
            ...
            bool success = std::all_of(
                    ...
                    [this, &slotsThisBucket, salt, &slotsClaimed](const TKeyValuePair& keyValuePair) -> bool {
                        ...
                        unsigned int slotWanted = Hash(key, strlen(key), (unsigned int)salt) % m_numItems;
...
int main (int argc, char **argv)
{
    ...
    for (decltype(table)::TKeyValuePair& keyValuePair : data) {
        ...
        if (value == nullptr)
        {
            cout << "Error, could not find data for key ", keyValuePair.first.c_str();
            return 0;
        }
        assert(*value == keyValuePair.second);
        if (*value != keyValuePair.second)
        {
            cout << "Incorrect value detected, should have gotten %i!n", keyValuePair.second;
            return 0;
        }
    }
